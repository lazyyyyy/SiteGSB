var idParcAuto = document.getElementById("parcAutoId").value;

var data = new FormData();
data.append("parc_auto_id", idParcAuto);
ajaxPost("http://localhost:8080/api/getParcAutoById.php", data, function(reponse){
    var ceParcAuto = JSON.parse(reponse);
    ...
    parcsAutos.forEach(function(parcAuto){
        if(parcAuto.libelle.toUpperCase() !== ceParcAuto.libelle)
            {
    ...
    var data2 = new FormData();
        ...
        ajaxPost("http://localhost:8080/api/getRegionByName.php", data2, function(reponse2){
            ...
            regions.forEach(function(region){
                ...
                if(region.id === ceParcAuto.lieu.region.id)
                    {
                        optionElt.selected = true;
                    }
...
/*    window.addEventListener("load", function(){
    ...
    document.getElementById("nom").value = ceParcAuto.libelle;
    document.getElementById("libelleLieu").value = ceParcAuto.lieu.id;
    console.log(ceParcAuto.lieu.pays);
    document.getElementById("adresselieu").value = ceParcAuto.lieu.adresse;
    document.getElementById("cpLieu").value = ceParcAuto.lieu.cp;
    document.getElementById("villeLieu").value = ceParcAuto.lieu.ville;
    document.getElementById("paysLieu").value = ceParcAuto.lieu.pays;
    document.getElementById("region").value = ceParcAuto.lieu.region.id;
        ...
        if(probleme)
            {
                ...
                if(document.getElementById("libelleLieu").value == "0")
                {
                    ...
                    ajaxPost("http://localhost:8080/api/getLieuByName.php", data2, function(reponse2){
                        ...
                        data3.append("id_parc_auto", idParcAuto);
                        ...
                        ajaxPost("http://localhost:8080/api/modifierParcAuto.php", data3, function(reponse2){
                            console.log("addParcAuto = " + reponse2);
                            var rep2 = JSON.parse(reponse2);
                            if(rep2)
                                {
                                    var repUtilisateur = alert("Le nouveau lieu ainsi que la modification du parc automobile ont bien été ajouté");
                ...
                else
                    {
                        ...
                        ajaxPost("http://localhost:8080/api/modifierParcAuto.php", data3, function(reponse2){
                            ...
                            if(rep2)
                                {
                                    alert("Modification Parc automobile effectuée");
                                    document.location.href = "parcauto.php?id=" + document.getElementById("region").value;
                                }
                            else{
