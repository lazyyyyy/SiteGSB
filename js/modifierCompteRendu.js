...
var idCompteRendu = document.getElementById("idCompteRendu").value;
...
var data = new FormData();
data.append("compte_rendu_id", idCompteRendu);
ajaxPost("http://localhost:8080/api/getCompteRenduById.php", data, function(reponse){
    var compteRendu = JSON.parse(reponse);
    document.getElementById("selectPraticien").value = compteRendu.praticien.id;
    document.getElementById("date").value = compteRendu.date;
    document.getElementById("motif").value = compteRendu.motif.id;
    document.getElementById("confiance").value = compteRendu.coef_confiance;
    document.getElementById("prescription").value = compteRendu.coef_prescription;
    document.getElementById("notoriete").value = compteRendu.coef_notoriete;
    document.getElementById("bilan").value = compteRendu.bilan;
    document.getElementById("produits").value = compteRendu.produit.id;
    document.getElementById("echantillons").value = compteRendu.nb_echantillons;
});
    ...
    if(document.getElementById("erreurDate").textContent === "")
        {
            ...
            data.append("id_compte_rendu", document.getElementById("idCompteRendu").value);
            ...
            ajaxPost("http://localhost:8080/api/modifierCompteRendu.php", data, function(reponse){
                console.log(reponse);
                var rep = JSON.parse(reponse);
                if(rep)
                    {
                        document.location.href = "compteRenduDetails.php?id=" + idCompteRendu;
                    }
                else{
